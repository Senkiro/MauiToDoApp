...
>>>>>>> ecfcb6d406a6bc260cc3e57a899bb2427741552f
    ...
    private async void OnEditClicked(object sender, EventArgs e)
    {
        ...
        if(item != null)
        {
            string newTitle = await DisplayPromptAsync("Sửa công việc", "Nhập nội dung mới:", initialValue: item.Title);
            if (!string.IsNullOrWhiteSpace(newTitle))
            {
                if(TodoItems.Any(x => x.Title.Equals(newTitle.Trim(),StringComparison.OrdinalIgnoreCase))){
                    await DisplayAlert("Lỗi", "Công việc này đã tồn tại.", "OK");
                    return;
                }
...
>>>>>>> 5514b3c8f86ed83973379affb8da5f76a248eba9
            }
        }
    }
    ...
    private async void OnItemSelected(object sender, SelectionChangedEventArgs e)
    {
        ...
        {
            await Navigation.PushAsync(new TodoDetailPage(selectedItem));

            // Bỏ chọn item sau khi điều hướng
